...
//! `newLibraryWithSource`), so no GPU SDK is needed to build. GPU support compiles
//! in unconditionally on every platform (no Cargo feature gate): the CUDA backend
//! everywhere `cudarc` targets (macOS never has an NVIDIA driver, so it's excluded
//! there), the Metal backend only on macOS. `cudarc` and `objc2-metal` themselves
//! dlopen/query the actual driver at runtime, so a machine with no GPU at all still
//! builds and runs fine -- `compile_devices` just returns an empty list.

mod kernel_source;

// CUDA: cudarc doesn't target macOS (no NVIDIA driver there), so this compiles on
// every other platform cudarc supports (Linux, Windows) unconditionally.
#[cfg(not(target_os = "macos"))]
pub(crate) mod cuda;
// Metal: the toolchain ships with every macOS SDK, so this compiles unconditionally
// on macOS targets.
#[cfg(target_os = "macos")]
pub(crate) mod metal;
...
/// individual devices that are present but unusable. Returns the ready devices (an
/// empty `Vec` -- never an error -- when no GPU is present or usable).
